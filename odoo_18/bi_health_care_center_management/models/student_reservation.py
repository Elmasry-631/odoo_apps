...
class StudentReservation(models.Model):
    ...
    _description = "Client Nurse Reservation"
        ...
        for reservation in reservations:
            if reservation.start_date and reservation.end_date:
                ...
                if current_date_local < start_date_local:
                    ...
                    #     Now we create an attendance for that reservation
                    employee_id = reservation.trainer_id.employee_id
                    if not employee_id:
                        employee_id = self.env['hr.employee'].sudo().search(
                            [('name', '=', reservation.trainer_id.name)])
    ...
    def action_change_details(self):
        ...
        view_id = self.env.ref('bi_health_care_center_management.reservation_change_wizard_form').id
